...
impl<'info> Make<'info> {
    pub fn make(&mut self, bumps: &MakeBumps, seed: u64, amount: u64) -> Result<()> {
        let escrow = &mut self.escrow;
        escrow.maker = *self.maker.key;
        escrow.mint_x = self.mint_x.to_account_info().key();
        escrow.mint_y = self.mint_y.to_account_info().key();
        escrow.amount = amount;
        escrow.seed = seed; 
        escrow.bump = bumps.escrow;     
        Ok(())
    }
